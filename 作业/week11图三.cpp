...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        AMGraph<T>* subgraph(T* vList, int n){ //ques 1
            ...
            VecList<T> tmp(vList,n);
            for(int i=0;i<numVer;i++)
            {
                for(int j=0;j<numVer;j++)
                {
                    if(adjMatrix[i][j]==0)continue;
                    int tmpi=tmp.locateEle(verList.getEleAtPos(i));
                    int tmpj=tmp.locateEle(verList.getEleAtPos(j));
                    if((tmpi!=-1)&&(tmpj!=-1))
                    {
                        result->addEdge(verList.getEleAtPos(i),verList.getEleAtPos(j));
                    }
                }
            }
        ...
        AMGraph<int>* getLineGraph(){
            // TODO
            int* arr=new int[numEdge];
            for(int i=0;i<numEdge;i++)
            {
                arr[i]=i+1;
            }
            AMGraph<int>* result = new AMGraph<int>(arr,numEdge);
            int adj[numEdge][2];
            int idx=0;
            for(int i=0;i<numVer;i++)
            {
            ...
            for(int i=0;i<numEdge;i++)
            {
                ...
                {
                    if(adj[j][0]==adj[i][0]||adj[j][1]==adj[i][0]||adj[j][0]==adj[i][1]||adj[j][1]==adj[i][1])
                        result->addEdge(i+1,j+1);
                }
            }
            return result;
        }
