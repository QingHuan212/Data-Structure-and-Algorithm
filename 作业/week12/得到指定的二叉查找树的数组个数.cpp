...
class BinarySearchTree{
    ...
    public:
        ...
        int treecount(BinTree<T>* tmp)
        {
            if(tmp==NULL)return 0;
            return 1+treecount(tmp->getLChild())+treecount(tmp->getRChild());
        }

        int countway(BinTree<T>* bt,int** comb,int modl)
        {
            if(bt==NULL)return 1;
            int leftcount=treecount(bt->getLChild());
            int rightcount=treecount(bt->getRChild());
            int leftway=mod(countway(bt->getLChild(),comb,modl),modl);
            int rightway=mod(countway(bt->getRChild(),comb,modl),modl);
            int way;
            way=mod(comb[leftcount+rightcount][leftcount]*leftway,modl);
            way=mod((way*rightway),modl);
            return way;
        }
        ...
        int numWaysToGetBST(int mod_num){
           ...
           for(int i=0;i<=nodeCount;i++)
           {
                ...
                for(int j = 0; j <= i; j++)
                {
                    ...
                    else comb[i][j] = mod((comb[i-1][j-1] + comb[i-1][j]),mod_num);
                }
           }
           int count=countway(root->getRChild(),comb,mod_num);
           for(int i=0;i<=nodeCount;i++)
           {
               delete[] comb[i];
           }
