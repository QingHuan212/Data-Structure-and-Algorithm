...
int missingAPterm(ArrayWrap<int> &aw){
	...
	int first=aw[0],last=aw[length-1];
	int cha=(last-first)/length;
	int low=0,high=length-1,mid;
	while(low<=high)
    {
        mid=(low+high)/2;
        int awmid=aw[mid];
        if(awmid==first+cha*mid)
        {
            low=mid+1;
        }
        else
...
int maxEleBitonic(ArrayWrap<int> &aw){
    ...
    while (low <= high)
     {
        mid = low + (high - low) / 2;
        if (aw[mid] > aw[mid-1] && aw[mid] > aw[mid+1])
        {
            return aw[mid];
        }
        if (aw[mid] > aw[mid-1])
