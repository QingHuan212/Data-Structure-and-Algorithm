...
void buildsolveBinTree(BinTree<int>* root,int st,int ed,int** split)
{
    if(st==ed) root->setData(st);
    else
    {
        root->setData(-1);
        BinTree<int>* left=new BinTree<int>;
        BinTree<int>* right=new BinTree<int>;
        buildsolveBinTree(left,st,split[st][ed],split);
        buildsolveBinTree(right,split[st][ed]+1,ed,split);
        root->addLChild(left);
        root->addRChild(right);
    }
}

void printsolveBinTree(BinTree<int>* root)
{
    if(root==NULL)return;
    if(root->getData()==-1)
    {
        cout<<"(";
        printsolveBinTree(root->getLChild());
        printsolveBinTree(root->getRChild());
        cout<<")";
    }
    else
    {
...
int minOpMatrixMult(int* sizes, int n){
    ...
    for(int l=1;l<n;l++)
    {
        for(int i=0;i<n-l;i++)
        {
            ...
            for(int s=i;s<j;s++)
            {
                int tmp=count[i][s]+count[s+1][j]+sizes[i]*sizes[s+1]*sizes[j+1];
                if(count[i][j]==-1||tmp<count[i][j])
                {
                    count[i][j]=tmp;
                    split[i][j]=s;
                }
    ...
    buildsolveBinTree(root,0,n-1,split);
    printsolveBinTree(root);
    cout<<endl;
    int tmp=count[0][n-1];
    for(int i=0;i<n;i++)
    {
