...
void radixDesc(MultiKey<T>** mk, int n, int keylen, int low, int high){
    ...
    {
        VecList<MultiKey<T>*>* sort = new VecList<MultiKey<T>*>[count];
        for(int j=0;j<n;j++)
        {
            int key = mk[j]->keys[i];
            sort[key-low].insertEleAtPos(sort[key-low].getLength(),mk[j]);
        }
        int pos = 0;
        for(int j=high-low;j>=0;j--)
        {
            if(sort[j].isEmpty())continue;
            for(int k=0;k<sort[j].getLength();k++)
            {
                mk[pos] = sort[j].getEleAtPos(k);
                pos++;
            }
        }
        delete[] sort;
