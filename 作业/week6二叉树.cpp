...
public:
    ...
    int leafNodeCount() { // question 1
        int count = 0;
        if (isLeaf())count++;
        if (left != NULL)count += left->leafNodeCount();
        if (right != NULL)count += right->leafNodeCount();
        return count;
    }
    ...
    bool isComplete() { // question 2
        ...
        while (!q.isEmpty()) {
            BinTree<T>* tmpN = q.deQueue();
            if (empty && tmpN != NULL)return false;
            if (tmpN == NULL)
            {
                empty = true;
                continue;
    ...
    void printSnakeLevelOrder() { // question 4
        ...
        while (!s1.isEmpty()||!s2.isEmpty())
        {
            if (depth % 2 == 1)
            {
                BinTree<T>* tmps1 = s1.pop();
                if (s1.getLength() == 0)depth++;
                cout << tmps1->data << " ";
                if (tmps1->left != NULL)s2.push(tmps1->left);
                if (tmps1->right != NULL)s2.push(tmps1->right);
            }
            else
            {
                BinTree<T>* tmps2 = s2.pop();
                if (s2.getLength() == 0)depth++;
                cout << tmps2->data << " ";
                if (tmps2->right != NULL)s1.push(tmps2->right);
                if (tmps2->left != NULL)s1.push(tmps2->left);
            }
        }
    }
...
BinTree<T>* buildFromPreIn(T* preArr, T* inArr, int n) {
    if (n == 0)return NULL;
    BinTree<T>* root = new BinTree<T>;
    root->setData(preArr[0]);
    int length = -1;
    T* inleft = new T[n];
    T* inright = new T[n];
    for (int i = 0; i < n; i++)
    {
        ...
        if (length == -1)inleft[i] = inArr[i];
        else inright[i - length - 1] = inArr[i];
    }
    T* preleft = new T[n];
    T* preright = new T[n];
    for (int i = 1; i < n; i++)
    {
        if (i - 1 < length)preleft[i - 1] = preArr[i];
        else preright[i - length - 1] = preArr[i];
    }
    root->addLChild(buildFromPreIn(preleft, inleft, length));
    root->addRChild(buildFromPreIn(preright, inright, n - length - 1));
    delete[] inleft;
    delete[] inright;
    delete[] preleft;
    delete[] preright;
...
void test3() {
    ...
    //int t2p[3] = { 1,2,3 };
    //int t2i[3] = { 2,1,3 };
    //int t2p[10] = {1,2,4,6,9,7,10,3,5,8};
    //int t2i[10] = {9,6,4,10,7,2,1,5,8,3};

    int t2p[6] = { 1,2,3,4,5,6 };
    int t2i[6] = { 2,4,6,5,3,1 };

    tmp2 = buildFromPreIn(t2p, t2i, 6);
    tmp2->printPreOrderR();
    cout << endl;
    tmp2->printInOrderR();
    cout << endl;
    ...
    //char t1p[9] = { 'A','B','C','D','E','F','G','H','I'};
    //char t1i[9] = { 'B','C','A','E','D','G','H','F','I'};
