...
int weightedPathLength(BinTree<int>* bt) {
    int k=0,depth = 0;
    LinkStack<BinTree<int>*> s1, s2;
    s1.push(bt);
    while (!s1.isEmpty() || !s2.isEmpty())
    {
        if (depth % 2 == 0)
        {
            BinTree<int>* tmp1 = s1.pop();
            if (tmp1->isLeaf())
            {
                k = k + tmp1->getData() * depth;
            }
            if (s1.getLength() == 0)depth++;
            if (tmp1->getLChild() != NULL)s2.push(tmp1->getLChild());
            if (tmp1->getRChild() != NULL)s2.push(tmp1->getRChild());
        }
        else
        {
            BinTree<int>* tmp2 = s2.pop();
            if (tmp2->isLeaf())
            {
                k = k + tmp2->getData() * depth;
            }
            if (s2.getLength() == 0)depth++;
            if (tmp2->getLChild() != NULL)s1.push(tmp2->getLChild());
            if (tmp2->getRChild() != NULL)s1.push(tmp2->getRChild());
        }
    }
    return k;
}
