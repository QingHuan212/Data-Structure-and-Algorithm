...
int minTimesForBuilding(int eggs, int numFloors){
    ...
    if (eggs == 0) return 301;
    for(int i=0;i <= 300;i++)
    {
        if (dp[eggs][i] >= numFloors) return i;
    }
    return 301;
}
...
int nextTestFloor(int hFloor, int eggsLeft, int lastSurvival,int lastBreak){
    ...
    leftfloor=hf-lf+1;
    int t=minTimesForBuilding(eggsLeft,leftfloor);
    if(t==0)return -1;
    int notset = 0;
    if (eggsLeft - 1 <= 0 || t - 1 <= 0) notset = 0;
    else notset = dp[eggsLeft - 1][t - 1];
    int candidate = lastSurvival + notset + 1;
    int next = 0;
    if (candidate < lf) next = lf;
    else if (candidate > hf)next = hf;
    else next = candidate;
    return next;
}
