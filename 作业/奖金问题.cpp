...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        void minbonus()
        {
            ...
            {
                int st=q.deQueue();
                for(int en=0;en<numVer;en++)
                {
                    if(adjMatrix[st][en]==0)continue;
                    pay[en]=max(pay[en],pay[st]+adjMatrix[st][en]);
                }
            }
            ...
            for(int i=0;i<numVer;i++)
            {
                cout << left << setw(10) <<  i+1 << pay[i] << endl;
            }
        }
};
int main()
{
    ...
    cin>>numVm;
    cin>>numEn;
    int* Ver = new int[numVm];
    for(int i=0;i<numVm;i++)
    {
        Ver[i]=i+1;
    }
    AMGraph<int> result(Ver,numVm,true);
    for(int i=0;i<numEn;i++)
    {
        int st,en,weight;
        cin>>st>>en>>weight;
        result.addEdge(st,en,weight);
    }
    result.minbonus();
    return 0;
}
