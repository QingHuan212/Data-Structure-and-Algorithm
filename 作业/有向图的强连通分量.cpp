...
class LGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        void firstdfs(int st,bool* visited,LinkStack<int>* first)
        {
            visited[st] = true;
            graphNode<T>* tmpNode = verList.getEleAtPos(st);
            for(int j=0;j<tmpNode->outs->getLength();j++)
            {
                int target = verList.locateEle(tmpNode->outs->getEleAtPos(j));
                if(visited[target]) continue;
                firstdfs(target,visited,first);
            }
            first->push(st);
        }

        void seconddfs(int st,bool* visited, VecList<int>* TransposeOut,VecList<int>* component)
        {
            visited[st]=true;
            component->insertLast(st);
            for(int i=0;i<TransposeOut[st].getLength();i++)
            {
                int target=TransposeOut[st].getEleAtPos(i);
                if(!visited[target])
                    seconddfs(target,visited,TransposeOut,component);
            }
        }

        void sortComponent(VecList<int>* component)
         {
            int n = component->getLength();
            bool change;
            for(int i = 0; i < n - 1; i++)
                {
                    change = false;
                    ...
                    {
                        int aIdx =component->getEleAtPos(j);
                        int bIdx = component->getEleAtPos(j + 1);
                        if(verList.getEleAtPos(aIdx)->data > verList.getEleAtPos(bIdx)->data) {
                            int tmp = aIdx;
                            component->setEleAtPos(j, bIdx);
                            component->setEleAtPos(j + 1, tmp);
                            change = true;
                        }
                    }
                    if(!change) break;
        ...
        void printStrongCC(){
            ...
            VecList<int>* TransposeOut=new VecList<int>[numVer];
            for(int i=0;i<numVer;i++)
            {
                graphNode<T>* tmpNode = verList.getEleAtPos(i);
                for(int k=0;k<tmpNode->outs->getLength();k++)
                {
                    int v = verList.locateEle(tmpNode->outs->getEleAtPos(k));
                    TransposeOut[v].insertLast(i);
                }
            }

            VecList<VecList<int>*> components;
            ...
            while(!first.isEmpty())
            {
                ...
                if(!visited[tmp])
                {
                    VecList<int>* component=new VecList<int>;
                    seconddfs(tmp,visited,TransposeOut,component);
            ...
            {
                sortComponent(components.getEleAtPos(i));
            }

            for(int i=0;i<components.getLength()-1;i++)
            {
                for(int j=i+1;j<components.getLength();j++)
                {
                    int aMinIdx = components.getEleAtPos(i)->getEleAtPos(0);
                    int bMinIdx = components.getEleAtPos(j)->getEleAtPos(0);
                    if(verList.getEleAtPos(aMinIdx)->data >
                    verList.getEleAtPos(bMinIdx)->data)
                   {
                        VecList<int>* tmp = components.getEleAtPos(i);
                        components.setEleAtPos(i, components.getEleAtPos(j));
                        components.setEleAtPos(j, tmp);
                   }
                }
            }
            ...
            {
                cout << "Component " << i << ": ";
                for(int j=0;j<components.getEleAtPos(i)->getLength();j++)
                {
                int idx = components.getEleAtPos(i)->getEleAtPos(j);
                cout << verList.getEleAtPos(idx)->data << " ";
                }
                cout << endl;
            }
