...
class LGraph{
    ...
    private:
        ...
        void BFShelper(int st, bool* visited){
            ...
            while(!q.isEmpty()){
                ...
                graphNode<T>* tmpNode = verList.getEleAtPos(tmpInd);
                cout << tmpNode->data << " ";
                VecList<graphNode<T>*>* tmpOuts = tmpNode->outs;
                for(int i=0;i<tmpOuts->getLength();i++){
                    graphNode<T>* destNode = tmpOuts->getEleAtPos(i);
                    int destInd = findIndex(destNode->data); // verList.getEleAtPos(destNode);
                    if(visited[destInd]) continue;
                    q.enQueue(destInd);
                    visited[destInd] = true;
                }
            }
		}
		...
		void DFShelper(int st, bool* visited){
            ...
            for(int i=0;i<tmpOuts->getLength();i++){
                graphNode<T>* destNode = tmpOuts->getEleAtPos(i);
                int destInd = findIndex(destNode->data);
                if(visited[destInd]) continue;
                DFShelper(destInd,visited);
            }
		}
    ...
    public:
        ...
        void removeEdge(T st, T en){ // st -> en
            int sInd = findIndex(st);
            int eInd = findIndex(en);
            graphNode<T>* sNode = verList.getEleAtPos(sInd);
            graphNode<T>* eNode = verList.getEleAtPos(eInd);
        ...
        int** getMatrix(){
            ...
            for(int i=0;i<numVer;i++){
                ...
                for(int j=0;j<numVer;j++){
                    graphNode<T>* eNode = verList.getEleAtPos(j);
                    if(sNode->outs->locateEle(eNode)!=-1) adjMatrix[i][j] = 1;
