...
class LGraph{
    ...
    private:
        ...
        void BFShelper(int st, bool* visited){
            ...
            visited[st] = true;
            while(!q.isEmpty()){
                int tmpInd = q.deQueue();
                graphNode<T>* tNode = verList.getEleAtPos(tmpInd);
                cout << tNode->data << " ";
                VecList<graphNode<T>*>* tOuts = tNode->outs;
                for(int i=0;i<tOuts->getLength();i++){
                    graphNode<T>* desNode = tOuts->getEleAtPos(i);
                    int desInd = verList.locateEle(desNode); // findIndex(desNode->data);
                    if(visited[desInd]) continue;
                    q.enQueue(desInd);
                    visited[desInd] = true;
                }
            }
		...
		void DFShelper(int st, bool* visited){
            visited[st] = true;
            graphNode<T>* tNode = verList.getEleAtPos(st);
            cout << tNode->data << " ";
            VecList<graphNode<T>*>* tOuts = tNode->outs;
            for(int i=0;i<tOuts->getLength();i++){
                graphNode<T>* desNode = tOuts->getEleAtPos(i);
                int desInd = verList.locateEle(desNode);
                if(visited[desInd]) continue;
                DFShelper(desInd,visited);
            }
		}
    ...
    public:
        ...
        ~LGraph(){
            for(int i=0;i<numVer;i++){
                graphNode<T>* delNode = verList.getEleAtPos(i);
                delete delNode->ins;
                delete delNode->outs;
                delete delNode;
            }
        ...
        void addEdge(T st, T en){ // st -> en
            ...
            // insert eNode into sNode->outs
            // insert sNode into eNode->ins
        ...
        void addVertex(T x){
            ...
            newNode->data = x;
            newNode->ins = new VecList<graphNode<T>*>;
            newNode->outs = new VecList<graphNode<T>*>;
            verList.insertLast(newNode);
        }
        ...
        int** getMatrix(){
            ...
            for(int i=0;i<numVer;i++){
                ...
                graphNode<T>* sNode = verList.getEleAtPos(i);
                for(int j=0;j<numVer;j++){
                    // adjMatrix[i][j]
                    graphNode<T>* desNode = verList.getEleAtPos(j);
                    if(sNode->outs->locateEle(desNode)!=-1) adjMatrix[i][j] = 1;
