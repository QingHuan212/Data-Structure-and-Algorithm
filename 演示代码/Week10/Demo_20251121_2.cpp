...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        void printDijkstra(T st){ // positive weights
            ...
            // ≥ı ºªØ
            for(int i=0;i<numVer;i++){
                dists[i] = (adjMatrix[sInd][i]==0 ? -1 : adjMatrix[sInd][i]);
                isInS[i] = false;
                lastIdx[i] = (adjMatrix[sInd][i]==0 ? -1 : sInd);
            }
            isInS[sInd] = true;
            dists[sInd] = 0;
            lastIdx[sInd] = -1;
            ...
            while(numS < numVer){
                ...
                for(int i=0;i<numVer;i++){
                    ...
                    if(dists[i]==-1 || (minDist + adjMatrix[minIdx][i] < dists[i])){
                        dists[i] = minDist + adjMatrix[minIdx][i];
                        lastIdx[i] = minIdx;
                    }
                }
            }
            ...
            for(int k=0;k<numVer;k++){
                for(int i=0;i<numVer;i++){
                    for(int j=0;j<numVer;j++){
                        ...
                        if(dists[i][j]==-1 || (dists[i][k] + dists[k][j] < dists[i][j])){
        ...
        T** preNodeDist(T bad){
            ...
            // positive weights
            int** dists = new int*[numVer];
            T** lastNodes = new T*[numVer];
            for(int i=0;i<numVer;i++){
                dists[i] = new int[numVer];
                lastNodes[i] = new T[numVer];
                for(int j=0;j<numVer;j++){
                    dists[i][j] = (adjMatrix[i][j]!=0 ? adjMatrix[i][j] : -1);
                    lastNodes[i][j] = (adjMatrix[i][j]!=0 ? verList.getEleAtPos(i) : bad);
                }
                dists[i][i] = 0;
                lastNodes[i][i] = bad;
            }
