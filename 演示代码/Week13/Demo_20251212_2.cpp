    ...
    public:
        ...
        bool deleteData(T x){
            ...
            if(xNode->getLChild()==NULL && xNode->getRChild()==NULL){
                ...
                else tmp->addRChild(xNode->getRChild());
                balanceTree(tmp);
                nodeCount--;
                delete xNode;
                return true;
                ...
                else tmp->addRChild(xNode->getLChild());
                balanceTree(tmp);
                nodeCount--;
                delete xNode;
                return true;
            }
            BinTree<T>* leftLargest = xNode->getLChild();
            BinTree<T>* leftLargestParent = xNode;
            while(leftLargest->getRChild()!=NULL){
                leftLargestParent = leftLargest;
                leftLargest = leftLargest->getRChild();
            }
            xNode->setData(leftLargest->getData());
            if(leftLargestParent!=xNode){
                leftLargestParent->addRChild(leftLargest->getLChild());
            }
            else leftLargestParent->addLChild(leftLargest->getLChild());
            balanceTree(leftLargestParent);
            nodeCount--;
            delete leftLargest;
            return true;
        }
        void printInOrder(){
            cout << "In order: ";
            if(nodeCount!=0) root->getRChild()->printInOrderR();
            cout << endl;
        }
        void printPreOrder(){
            cout << "Pre order: ";
            if(nodeCount!=0) root->getRChild()->printPreOrderR();
            cout << endl;
        }
        void printPostOrder(){
            cout << "Post order: ";
            if(nodeCount!=0) root->getRChild()->printPostOrderR();
            cout << endl;
        }

        void printLevelOrder(){
            cout << "Level order: ";
            if(nodeCount!=0) root->getRChild()->printLevelOrder();
            cout << endl;
        }
};
