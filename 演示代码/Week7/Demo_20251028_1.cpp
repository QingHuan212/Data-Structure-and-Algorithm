...
BinTree<HTNode*>* buildHuffmanTree(char* charSet, int* freqSet, int count){
    ...
    // Step 1: make a node for each character.
    ...
    // merge until only one tree left
    while(treeCount > 1){
        // merge forest[treeCount-1], forest[treeCount-2]
        HTNode* ht = new HTNode;
        ht->isChar = false;
        ht->freq = forest[treeCount-1]->getData()->freq + forest[treeCount-2]->getData()->freq;
        BinTree<HTNode*>* bt = new BinTree<HTNode*>(ht);
        bt->addLChild(forest[treeCount-2]);
        bt->addRChild(forest[treeCount-1]);

        // put the new tree in the right place
        int j;
