...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
        ...
        void printBFShelp(int vInd, bool* visited){
            ...
            while(!Q.isEmpty()){
                ...
                for(int j=0;j<numVer;j++){
                    if(adjMatrix[start][j]==0) continue;
    ...
    public:
        ...
        void addEdge(T st, T en){
            int sIdx = verList.locateEle(st);
            int eIdx = verList.locateEle(en);
            if(adjMatrix[sIdx][eIdx]!=0) return;
            numEdge++;
            adjMatrix[sIdx][eIdx] = 1;
            if(!directed) adjMatrix[eIdx][sIdx] = 1;
        }
        ...
        void removeEdge(T st, T en){
            ...
            if(adjMatrix[sIdx][eIdx]==0) return;
            numEdge--;
            adjMatrix[sIdx][eIdx] = 0;
            if(!directed) adjMatrix[eIdx][sIdx] = 0;
        }
        void printGraph(){
            cout << "Vertices: " << endl;
            for(int i=0;i<numVer;i++){
            ...
            char leftP = (directed ? '<' : '(');
            char rightP = (directed ? '>' : ')');
            for(int i=0;i<numVer;i++){
                for(int j=(directed ? 0 : i+1);j<numVer;j++){
                    if(adjMatrix[i][j]==0) continue;
                    cout << leftP << verList.getEleAtPos(i) << "," << verList.getEleAtPos(j) << rightP << endl;
                }
            }
        }
        int** getMatrix(){
