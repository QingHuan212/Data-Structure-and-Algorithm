...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    private:
        ...
        void printBFShelp(int vInd, bool* visited){
            ...
            while(!q.isEmpty()){
                int out = q.deQueue();
                cout << verList.getEleAtPos(out) << " ";
                for(int j=0;j<numVer;j++){
                    if(adjMatrix[out][j]==0) continue;
    ...
    public:
        ...
        void addEdge(T st, T en){
            ...
            int eInd = verList.locateEle(en);
            // modify adjMatrix[sInd][eInd]
            if(adjMatrix[sInd][eInd]!=0) return;
            numEdge++;
            adjMatrix[sInd][eInd] = 1;
            if(!directed) adjMatrix[eInd][sInd] = 1;
        }
        ...
        void removeEdge(T st, T en){
            ...
            if(adjMatrix[sInd][eInd]==0) return;
            numEdge--;
            adjMatrix[sInd][eInd] = 0;
            if(!directed) adjMatrix[eInd][sInd] = 0;
        }
        void printGraph(){
            cout << "Vertices:" << endl;
            for(int i=0;i<numVer;i++){
            ...
            char leftS = (directed ? '<' : '(');
            char rightS = (directed ? '>' : ')');
            for(int i=0;i<numVer;i++){
                for(int j=(directed ? 0 : i+1);j<numVer;j++){
                    if(adjMatrix[i][j]==0) continue;
                    cout << leftS << verList.getEleAtPos(i) << "," << verList.getEleAtPos(j) << rightS << endl;
                }
            }
        }
        int** getMatrix(){
